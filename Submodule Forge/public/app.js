...
function syncSubmodulePath(force = false) {
  ...
  const current = submodulePathInput.value.trim();
  if (force || current === '' || current === autoSubmodulePath) {
...
function syncBaseFolderName(force = false) {
  if (!baseFolderInput) {
    return;
  }
  ...
  const current = baseFolderInput.value.trim();
  if (force || current === '' || current === autoBaseFolder) {
...
function addSubmoduleRow() {
  ...
  repoIn.addEventListener('input', () => {
    ...
    if (repoName && pathIn.value.trim() === '') pathIn.value = repoName;
  });
...
function resetUiState() {
  ...
  output.textContent = t('log.placeholder');
  setStatus('idle');
  syncBaseFolderName(true);
  syncSubmodulePath(true);
  updateTokenToggleLabel();
  setActiveTab('setup');
}
