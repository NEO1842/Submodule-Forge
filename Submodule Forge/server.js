...
async function handleSetup(req, res) {
  ...
  try {
    ...
    for (let i = 0; i < submodules.length; i++) {
      ...
      // Git uses forward slashes for paths even on Windows
      const submoduleRelative = path.relative(baseDir, submoduleDir).split(path.sep).join('/');
      ...
      logs.push(`Submodule ${i + 1} added and staged.`);
    }
